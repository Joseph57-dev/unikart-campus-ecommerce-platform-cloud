...
// Sidebar toggle and category click handling (mobile-friendly)
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (sidebar) {
        if (window.innerWidth <= 991) sidebar.classList.add('mobile-hidden');
    }
    ...
    // Desktop collapse button: shrink to icons
    const collapseBtn = document.getElementById('sidebar-collapse-btn');
    if (collapseBtn && sidebar) {
        // set data-title attributes for tooltip use
        document.querySelectorAll('.modern-category-item').forEach(item => {
            const label = item.querySelector('span');
            const anchor = item.querySelector('a');
            if (label) {
                const text = label.textContent.trim();
                item.setAttribute('data-title', text);
                if (anchor) anchor.setAttribute('title', text);
            }
        });

        // Toggle collapse on desktop, close sidebar on mobile
        collapseBtn.addEventListener('click', function(e) {
            if (window.innerWidth <= 991) {
                // On mobile treat collapse button as close
                closeSidebar();
                return;
            }
    ...
    // Intercept category links to smooth-scroll to first matching product card
    document.querySelectorAll('.modern-category-list a').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            try {
                ...
                if (!category) {
                    const target = document.querySelector('.section-header') || document.querySelector('.welcome-banner');
                    if (target) {
                        e.preventDefault();
                        closeSidebar();
                ...
                const selector = `.modern-product-card[data-category="${category}"]`;
                ...
                // else: let link perform a full page navigation to server-filtered view
            } catch (err) {
                // If URL parsing fails, allow default link behavior
            }
        });
    });
});
